...
function assertFullMutationHistoryInvariant(task) {
  ...
  const proof = task.coverage_intervals.find((interval) => (
  ...
  if (!proof || !task.capability_leases.some((lease) => lease.operation_id === proof.operation_id && lease.capability_id === proof.capability_id)) {
    throw new Error("full mutation history requires an exhaustive operation-scoped capability lease");
  }
}
