...
import { REVIEW_LEVELS, STUCK_REPEATS, TOUCHED_FILES_CAP, fnv1aHex } from "./prims.mjs";
...
function ensureEpisode(task, session, at) {
  ...
  episode = { id: task.episodes.length + 1, session: sid, opened_at: at };
...
function transition(task, event) {
  ...
  switch (event.type) {
    case "begin-episode": {
      const episode = ensureEpisode(next, event.session, at);
