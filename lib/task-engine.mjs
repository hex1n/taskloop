...
function assertFullOperationReceiptInvariant(task) {
  ...
  for (const interval of task.coverage_intervals.filter((item) => item.mutation_history === "full")) {
    ...
    if (!operation?.authorization || operation.authorization.host_profile !== interval.host_profile || (
      interval.interval_to_checkpoint !== null && (!operation.completion || operation.completion.host_profile !== interval.host_profile)
...
const V3_REDUCERS = {
  ...
  coverage_changed(state, event) {
    ...
    if (payload.mutation_history === "full" && payload.interval_to_checkpoint === null && Object.values(state.operations).some((operation) => operation.authorization === null && operation.completion !== null)) {
      throw new Error("full mutation history cannot be established after an orphan completion receipt");
    }
