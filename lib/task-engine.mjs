...
function assertCurrentAuthorityProjection(value) {
  ...
  for (const task of value.tasks) {
    currentAuthorityRequire(hasExactKeys(task, ["task_id", "attachment_id", "goal", "write_claims", "placement", "branch_intent", "base_oid", "placement_intent_digest", "coordinator_session_id", "participant_session_ids", "lifecycle", "attachment_final_digest", "operation_intents", "tool_completions", "git_receipts"]), "task fields");
    ...
    currentAuthorityRequire(v3Integer(task.operation_intents) && v3Integer(task.tool_completions) && Array.isArray(task.git_receipts), "task receipt counters");
  }
