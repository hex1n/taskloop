...
function assertFullMutationHistoryInvariant(task) {
  ...
  if (Object.values(task.operations).some((operation) => operation.authorization === null && operation.completion !== null)) {
...
function assertFullOperationReceiptInvariant(task) {
  ...
  for (const interval of task.coverage_intervals.filter((item) => item.mutation_history === "full")) {
    ...
    if (!operation?.authorization || operation.authorization.host_profile !== interval.host_profile ||
...
function episodeCoverageBoundaryEvent(state, command, episode) {
  ...
  return coverageChangedEvent(command, {
    ...
    operationId: null,
    capabilityId: null,
    hostProfile: "runtime-contract-6",
...
const V3_DECIDERS = {
  ...
  resume(state, command) {
    ...
    if (taskRuntimeContract(state) === 6) events.push(episodeCoverageBoundaryEvent(state, command, episode));
...
const V3_REDUCERS = {
  ...
  tool_completed(state, event) {
    ...
    if (operation.authorization !== null && (
      operation.authorization.tool_family !== completion.tool_family || operation.authorization.host_profile !== completion.host_profile
    )) throw new Error("tool_completed does not match its operation authorization");
  ...
  artifact_reconciled(state, event) {
    ...
    if (payload.to_checkpoint !== payload.from_checkpoint) {
      ...
      if (state.authority.prewrite_enforcement === "full") state.authority.prewrite_enforcement = "unknown";
