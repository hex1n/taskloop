...
function validateProjectionRow(row) {
  ...
  if (![3, 4].includes(row.projection_schema_version)) throw new Error("invalid outcome-v3 projection version");
...
function observeWriteEvidence(evidenceByTask, event) {
  ...
  if (event.kind === "task_opened") {
    evidence.runtime_contract = event.payload?.runtime_contract === 6 ? 6 : 5;
  ...
  if (event.kind === "artifact_reconciled") {
    if (event.payload?.from_checkpoint !== event.payload?.to_checkpoint) evidence.artifact_changes += 1;
...
function rowsForRecords(repoIdentity, records, { seedRecords = [], seedRows = [] } = {}) {
  ...
  for (const record of records) for (const event of record.events) {
    ...
    const projectionSchemaVersion = evidence.runtime_contract === 6 || V3_OUTCOME_PROJECTION_SCHEMA_VERSION >= 4 ? 4 : 3;
