...
const EXPLICIT_PROFILES = Object.freeze(["claude", "codex"]);
...
const HOST_PROFILE_CAPABILITIES = Object.freeze({
  ...
  codex: Object.freeze({
    ...
    capability_id: "hostcap:v1:codex-direct", exhaustive_surface: false,
    completion_events: Object.freeze(["PostToolUse"]), receipt_quality: "tool_specific",
  }),
});
const RELEASE_ONLY_HOLD_SUFFIXES = Object.freeze({
  codex: "Codex is release-only; continue with an external driver or run workloop certify explicitly",
});

function encoded(stdout = "", stderr = "") { return { stdout, stderr, exitCode: 0 }; }

function assertProfile(profile) {
  if (!EXPLICIT_PROFILES.includes(profile)) throw new Error(`unsupported hook profile: ${profile}`);
  return profile;
}
