...
function repositoryContentFiles(repo) {
  const files = [];
...
function repoSnapshot(repo, previous = null) {
  ...
  const listed = repositoryContentFiles(repo);
  ...
  for (const file of files) {
    ...
    try {
      ...
      const content = stat.isSymbolicLink() ? `link:${fs.readlinkSync(absolute)}` : fs.readFileSync(absolute, "latin1");
      entries.push(reused ?? `${file}\0${sha256Hex(content)}`);
    } catch {
      ...
      return { hash: null, files: [], error: new Error(`repository snapshot unavailable at ${file}`) };
    }
  }
...
function runCriterionSource(source, repo, timeoutSeconds = CRITERION_TIMEOUT_SECONDS, protocol = "binary", {
  ...
  const deadlineTimeoutMs = Number.isSafeInteger(deadlineEpochMs) ? deadlineEpochMs - started : configuredTimeoutMs;
  const executionTimeoutMs = Math.max(1, Math.min(configuredTimeoutMs, deadlineTimeoutMs));
  let result;
  if (Number.isSafeInteger(deadlineEpochMs) && deadlineTimeoutMs <= 0) {
    ...
    try {
      ...
      result = spawnSync(invocation.executable, invocation.args, { cwd: repo, encoding: "utf8", env: spawnEnv(), timeout: executionTimeoutMs, maxBuffer: 10 * 1024 * 1024 });
    } catch (error) {
    ...
    const env = spawnEnv();
    result = shell
      ? spawnSync(command, { cwd: repo, encoding: "utf8", env, shell: true, timeout: executionTimeoutMs, maxBuffer: 10 * 1024 * 1024 })
      : spawnSync(argv[0], argv.slice(1), { cwd: repo, encoding: "utf8", env, timeout: executionTimeoutMs, maxBuffer: 10 * 1024 * 1024 });
  ...
  onSnapshots?.({ before_hash: before.hash, after_hash: after.hash });
