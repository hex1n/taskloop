...
function artifactCheckpointFromSnapshot(snapshot) {
  ...
  const entries = (snapshot.entries ?? []).map((entry) => {
    ...
    if (separator <= 0) throw new Error("repository snapshot entry is invalid");
    const entryPath = entry.slice(0, separator);
    const rawHash = entry.slice(separator + 1);
  ...
  return { checkpoint_id: artifactCheckpointId(entries), entries };
}

function artifactCheckpointDelta(before, after) {
  const previous = new Map((before?.entries ?? []).map((entry) => [entry.path, entry.hash]));
  const current = new Map((after?.entries ?? []).map((entry) => [entry.path, entry.hash]));
