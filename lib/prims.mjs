...
// persisted contracts and the authority-state validators); a schema change
...
// these exact fields before authority-state receives an event, and authority-state then
...
const CURRENT_AUTHORITY_EVENT_PAYLOAD_FIELDS = Object.freeze({
  ...
  authority_tail_recovered: Object.freeze(["authority_id", "valid_end_offset", "discarded_sha256", "granted_by", "reason"]),
  ...
  task_certified: Object.freeze(["task_id", "attachment_id", "session_id", "prepared_sequence", "attachment_final_digest", "criterion_digest", "commit_oid", "reason", "granted_by"]),
  ...
  git_stage_receipted: Object.freeze(["task_id", "attachment_id", "head_oid", "index_before", "index_after", "paths", "clean", "status", "granted_by", "reason"]),
  git_commit_receipted: Object.freeze(["task_id", "attachment_id", "prior_head", "commit_oid", "parent_oid", "diff_paths", "index_after", "clean", "status", "granted_by", "reason"]),
});
...
// (authority state) and the deny text (application) must name the same label and
