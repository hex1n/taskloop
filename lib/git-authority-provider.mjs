...
const CONTROL_TOP_LEVEL = Object.freeze(new Set([".git", ".workloop", ".workloop-incompatible-archive"]));
...
function resolveGitAuthorityTarget(target) {
  ...
  const controls = [path.join(worktreeRoot, ".git"), path.join(worktreeRoot, ".workloop"), path.join(worktreeRoot, ".workloop-incompatible-archive")];
...
function isControlRelative(value) {
  return CONTROL_TOP_LEVEL.has(String(value ?? "").split("/")[0]);
}
...
function canonicalClaimPath(discovery, value, kind) {
  ...
  if (!raw || (!repositoryRoot && (normalized === "." || path.posix.isAbsolute(normalized) || /^[A-Za-z]:\//u.test(normalized) || /[*?[\]{}]/u.test(normalized) || lexicalParts.some((part) => !part || part === "." || part === "..") || CONTROL_TOP_LEVEL.has(lexicalParts[0])))) {
  ...
  if ((relative === "" && !repositoryRoot) || (repositoryRoot && relative !== "") || relative === ".." || relative.startsWith("../") || path.posix.isAbsolute(relative) || CONTROL_TOP_LEVEL.has(canonicalParts[0])) {
...
function claimContains(claim, target) {
  if (isControlRelative(target)) return false;
...
function targetRelative(discovery, target) {
  ...
  const normalized = relative === "" ? "." : foldCasePath(relative);
  if (relative === ".." || relative.startsWith("../") || path.posix.isAbsolute(relative) || isControlRelative(normalized)) throw providerError("TARGET_SCOPE_UNAVAILABLE", "target is outside the selected Git worktree or is Workloop control state");
  return normalized;
}
...
function project(discovery, evolveAllCurrentAuthority, suppliedRecords = null, selection = {}) {
  ...
  const observedCollision = Boolean(attachment && currentLocator?.state === "claimed" && currentLocator.record_digest === attachment.claimed_locator_digest && currentLocator.authority_id === state.authority_id && currentLocator.attachment_id === attachment.attachment_id && currentLocator.claim_epoch === attachment.claim_epoch && currentLocator.anchor_id === attachment.anchor_id && !anchorMatches);
...
function lockManagerFor(discovery, createLockManager) {
  return createLockManager({
    resolveLockPath: ({ lockClass, resourceId }) => path.join(
      ...
      `.workloop-${lockClass === "maintenance" ? "authority" : lockClass}-${sha256Hex(resourceId).slice(7, 23)}.lock`,
    ),
...
function queryCurrentGit(target, kind = "status", { createLockManager, evolveAllCurrentAuthority }, selection = {}) {
  ...
  const warnings = [];
  const records = readAuthority(discovery, evolveAllCurrentAuthority);
  const value = project(discovery, evolveAllCurrentAuthority, records, selection);
...
function gitTaskPaths(task) {
  const paths = task.write_claims.map((claim) => claim.path === "." ? "." : claim.path);
  if (!paths.includes(".")) return paths;
  return [...paths, ":(exclude).workloop/**", ":(exclude).workloop-incompatible-archive/**"];
}
...
function receiptTask(discovery, records, input, evolveAllCurrentAuthority) { const state = evolveAllCurrentAuthority(null, records); const value = project(discovery, evolveAllCurrentAuthority, records, { taskId: input.taskId, sessionId: input.sessionId }); const task = state?.tasks.find((item) => item.task_id === input.taskId) ?? null; if (!task || task.attachment_id !== value.attachment_id || value.attachment?.availability !== "available" || !task.participant_session_ids.includes(input.sessionId)) throw providerError("TASK_ATTACHMENT_MISMATCH", "Git receipt target does not own an available participant task"); if (!value.routable || task.lifecycle.state !== "active") throw providerError("TASK_NOT_ACTIVE", "Git receipt operations require an active task"); return { state, value, task }; }
...
function recoverCurrentGitAuthorityTail(input, runtime) {
...
function exportCurrentGitAuthority(input, runtime) {
  const discovery = resolveGitAuthorityTarget(input.target);
  return exportAuthorityArtifact({
    ...
    forbiddenRoots: [discovery.worktree_root, discovery.git_common_dir, discovery.authority_root],
    grantedBy: input.grantedBy,
    reason: input.reason,
  });
}
...
export { mutateCurrentGitTask, openCurrentGitTask, queryCurrentGit, recordCurrentGitHook, recoverCurrentGitAttachment, recoverCurrentGitAuthorityTail, exportCurrentGitAuthority, forkCurrentGitIdentity, resolveGitAuthorityTarget, stageCurrentGitTask, commitCurrentGitTask, prepareCurrentGitCertification, certifyCurrentGitTask };
