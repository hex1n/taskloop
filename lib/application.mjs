...
function hookPretool(invocation, repo, callAnalysis) {
  ...
  try { return withTaskLock(repo, () => {
    ...
    commitTaskCommand(repo, { type: "authorize-write", taskId: current.task_id, decision: "allow", reason: "", transcriptRange: range, files: [...(targets.length ? targets : ["<command>"]), ...commandMarkers], actingSession, at, atEpochMs }, { actorKind: "hook", authority, commandId });
    return allowTaskloopCommand(sessionId, mapping, tool, invocation.agentId, callAnalysis);
  }); }
  catch (error) {
