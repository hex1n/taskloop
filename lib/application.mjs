...
function closeAttempt(repo, { explicit, stop = false }) {
  return withTaskLock(repo, () => {
    ...
    const at = now();
    const result = transition(task, { type: explicit ? "achieve" : "observe", source: explicit ? "achieve" : "stop", observation, drift, attemptId: randomUUID(), signature: fnv1aHex(observation.execution.output_tail), failureSummary: outputTail(observation.execution.output_tail, 160), at });
    ...
    if (!explicit && observation.verdict === "satisfied" && result.meta.closure?.state === "eligible" && next.policy.close_policy === "automatic") {
      next = transition(next, { type: "achieve", source: "stop", observation: { ...observation, observation_id: randomUUID() }, drift, at }).task;
    }
    saveTask(repo, next);
