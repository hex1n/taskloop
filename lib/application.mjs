...
import { criterionDrift, criterionMessage, criterionMetadata, repoSnapshot, resolveCriterionFile, resolveSubject, runCriterionSource } from "./criterion.mjs";
...
function prepareTaskObservation(repo, {
  intent,
  stop = false,
  sessionId = null,
  transcriptPath = null,
  ...
  return withTaskLock(repo, () => {
    ...
    return {
      ...
      repository_hash: repoSnapshot(repo).hash,
      transcript_range: stop ? transcriptRange(task, transcriptPath) : null,
    };
  });
}

function executePreparedObservation(repo, prepared, { intent, runtimeDeadlineSeconds = null } = {}) {
  ...
  const deadlineEpochMs = startedAtEpochMs + deadlineSeconds * 1000;
  ...
  return { status: "executed", ...lease.value };
}

function commitPreparedObservation(repo, prepared, executed, action) {
  return withTaskLock(repo, () => {
    const authority = loadV3Authority(repo, { recoverTail: true });
    const current = authority.projection;
    if (!sameObservationAuthority(prepared.token, observationAuthorityToken(authority, current, prepared.intent))) return { status: "stale" };
    const sideEffect = executed.observation.execution.execution_error === "criterion_side_effect";
    if (prepared.repository_hash === null || executed.snapshots?.before_hash !== prepared.repository_hash) return { status: "stale" };
    const currentRepositoryHash = repoSnapshot(repo).hash;
    if (executed.snapshots?.after_hash === null || executed.snapshots?.after_hash === undefined || currentRepositoryHash === null) return { status: "stale" };
    if (sideEffect) {
      ...
      return { status: "stale", evidence: action({ authority, task: current }) };
    }
    if (executed.snapshots.after_hash !== prepared.repository_hash || currentRepositoryHash !== prepared.repository_hash) return { status: "stale" };
...
function cmdOpen(values) {
  ...
  const intent = "open";
  const prepared = withTaskLock(repo, () => {
    const authority = loadV3Authority(repo, { recoverTail: true });
  ...
  const executed = executePreparedObservation(repo, prepared, { intent });
...
function cmdVerifyRecord(repo) {
  ...
  if (prepared.status === "criterion_in_progress") return error(criterionInProgressMessage(prepared.lease));
  if (prepared.status !== "ready") return error("verify --record requires an active task; resume first");
...
function adjudicateClosure(repo, { explicit, stop, sessionId, actorSession = null, transcriptPath, inlineCriterionBudgetSeconds = null }, readiness) {
  ...
  const prepared = prepareTaskObservation(repo, { intent, stop, sessionId, transcriptPath, inlineCriterionBudgetSeconds }, readiness);
  ...
  const finalized = commitPreparedObservation(repo, prepared, executed, ({ authority, task }) => {
    ...
    if (observation.execution.execution_error === "criterion_side_effect") {
      return {
        kind: "side_effect",
        committed: commitTaskCommand(repo, { type: "criterion-side-effect", taskId: task.task_id, observation, transcriptRange: prepared.transcript_range, actingSession: stop ? sessionId : hostSessionId(), at, atEpochMs }, { actorKind: stop ? "hook" : "cli", authority }),
      };
    }
    const drift = criterionDrift(task.criterion, repo);
