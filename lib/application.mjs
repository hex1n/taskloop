...
import { activeEpisode, spentTokens, touchedSummary, resumeBanner, createTask, transition } from "./task-engine.mjs";
...
function hookPretoolLocked(payload, repo, currentTask) {
  const episode = transition(currentTask, { type: "begin-episode", at: utcNow(), session: payload.session_id });
  ...
  if (isPlainObject(task.suspension)) {
    saveTask(repo, task);
    return deny(
      `taskloop: task is suspended (${task.suspension.outcome ?? "unknown"}); reads and verification remain free. ` +
...
function hookStopLocked(payload, repo, currentTask) {
  // A suspended task is deliberately parked: writes are frozen and it waits for
  // a human (needs_input) or is paused after a machine stall. Re-running the
  // criterion here would burn a round and re-block a task that already stopped
  // on purpose — observed live when a /goal driver kept re-injecting the goal
  // after an honest needs_input suspension and drained the whole budget. Release
  // the stop untouched: reads and verification stay free, and only
  // resume --reason (after amend, if direction changed) moves the task forward.
  if (isPlainObject(currentTask.suspension)) return 0;
  const episode = transition(currentTask, { type: "begin-episode", at: utcNow(), session: payload.session_id });
  let task = episode.task;
  tallyEpisodeTokens(task, payload);
  if (episode.meta.resumed) process.stderr.write(resumeBanner(task));
