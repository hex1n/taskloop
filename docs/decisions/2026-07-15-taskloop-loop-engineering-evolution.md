...
> ⛔ **2026-07-16：charter 已被 owner 拒绝。`decision: BUILD` 作废。** 裁决见 [Gate 0 裁决记录](2026-07-16-supervisor-charter-gate0-verdict.md)。
> **本稿自己的三个开关全部开火**：**§13 五条「会成为最差方案」的条件命中五条**；**§11 脚注**（定价明显超过真实事故成本 → 退回 DEFER）；**§14 末行**（不许在没有对照 eval 时宣布 supervisor 优于 gate）。
> **仍然有效**：§4（现状盘点）、§13、§14。**§11 支撑项「显式 argv criterion」是全篇唯一有硬证据的条目，已独立存活并由[票 10](2026-07-16-criterion-transport-form.md) 裁完。** **重开条件**（票 08 已生效裁决）：① 出现单会话装不下的任务；② 僵死在一个月内吃掉 ≥2 个夜晚。
>
> 日期：2026-07-15
> 模式：方案
> 深度：深度
...
- 当前实现源码：`lib/application.mjs`、`历史任务状态运行时`、`lib/event-store.mjs`、`lib/task-store.mjs`、`lib/criterion.mjs`、`历史监督运行时`、`lib/host-hooks.mjs`。
...
| 维度 | 当前实现证据 | 判断 |
|---|---|---|
| 终态验证 | fresh observation、criterion drift、proof gap、review freshness；`历史任务状态运行时:100-197,685-715` | 强，应该保留为内核 |
| 停止与预算 | rounds、writes、wall clock、output tokens、重复失败和无 artifact 进展；`历史任务状态运行时:206-221,471-491` | 强，可直接成为 supervisor policy |
...
| 结构化领域核 | `decide/evolve` 已接收结构化 command 并返回 events/result；`历史任务状态运行时:518+` | 强，说明去 shell 不必重写领域规则 |
...
| 安全 | grants 与保守拒绝有价值，但源码明确说明不是 sandbox；`历史监督运行时:129-163` | 只能是 policy signal |
