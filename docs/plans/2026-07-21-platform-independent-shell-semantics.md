...
- 深度：Deep
- 状态：已收敛，尚未授权实施
- 输入来源：现场 Codex 转录、当前 `lib/host-hooks.mjs`、`lib/application.mjs`、`历史监督运行时`、Windows/对抗测试及既有 Hook 设计文档
...
- `application.mjs` 和 `历史监督运行时` 可以分别维护命令语法判断。
...
- `历史监督运行时`：token IR、redirection 分类和 canonical analysis。
