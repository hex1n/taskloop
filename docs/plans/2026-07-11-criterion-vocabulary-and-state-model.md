...
**依据**：`lib/criterion.mjs`、`历史任务状态运行时`、`lib/application.mjs`、`README.md`、`skills/loop-core/REFERENCE.md`、`skills/workloop/SKILL.md` 现状实读
...
- `历史任务状态运行时` 与 outcome ledger 持久化 `earn_red`、`red_witnessed`、`keep_green` 等字段。
...
| `历史任务状态运行时` | 采用规范 lifecycle、policy、witness 与 closure 判断 |
