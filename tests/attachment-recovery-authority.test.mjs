...
function open(fx) { return json(run(fx, ["open", "--target", path.join(fx.root, "src", "future.txt"), "--filesystem-root", fx.root, "--goal", "ticket08 fixture", "--write-root", "src", "--command-id", "ticket08-open", "--reason", "fixture", "--granted-by", "self"])); }
...
test("filesystem copied identity enters collision, explicit reattach advances epoch, and command replay is idempotent", (t) => {
  ...
  const collision = json(run(fx, ["status", "--target", path.join(copied, "src", "next.txt")], "ticket08-copy"));
  assert.equal(collision.routable, false); assert.equal(collision.routing_reason, "attachment_collision");
  const args = ["reattach", "--target", path.join(copied, "src", "next.txt"), "--attachment", opened.attachment_id, "--command-id", "ticket08-reattach", "--expect-epoch", "1", "--expect-locator-digest", oldLocator.record_digest, "--reason", "select copied root", "--granted-by", "user"];
  ...
  const original = json(run(fx, ["status", "--target", path.join(fx.root, "src", "next.txt")], "ticket08-original"));
...
test("filesystem identity fork creates a fresh detached authority without copying source tasks", (t) => {
  ...
  const collision = json(run(fx, ["status", "--target", path.join(copied, "src", "next.txt")], "ticket08-fork")); assert.equal(collision.routing_reason, "attachment_collision");
  const args = ["fork-identity", "--target", path.join(copied, "src", "next.txt"), "--attachment", opened.attachment_id, "--command-id", "ticket08-fork", "--expect-epoch", "1", "--expect-locator-digest", old.record_digest, "--reason", "select copied identity as a new authority", "--granted-by", "user"];
  ...
  const source = json(run(fx, ["status", "--target", path.join(fx.root, "src", "next.txt")], "ticket08-source")); assert.equal(source.routable, false);
});
